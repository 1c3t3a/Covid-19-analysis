...
class CovidCasesWHOv1(CovidCases):
    ...
    Returns:
        CovidCasesWHOv1: A class to provide access to some data based on the WHOv1 file.
    """

    def __init__(self, filename):
        """The constructor takes a string containing the full filename of a CSV
        database you can download from the GitHub WHO website:
        https://github.com/WorldHealthOrganization/xform-covid-casecount-who/tree/master/output/admin0
        ...
        for geoID in self.__df['GeoID'].unique():
            ...
            # the cumulated cases
            cumCases = dfSingleCountry['CumCases'].to_numpy(copy=True) 
            # the cumulated cases to be shifted by one day
            cumCasesShifted = dfSingleCountry['CumCases'].to_numpy(copy=True) 
            # delete the last element
            cumCasesShifted = np.delete(cumCasesShifted, [len(cumCasesShifted) - 1])
            # insert a 0 as the first element
            cumCasesShifted = np.insert(cumCasesShifted, 0, 0)
            # subtract the two to get the new cases
            cases = cumCases - cumCasesShifted
            # add them to the dataframe
            dfSingleCountry['DailyCases'] = cases

            # the cumulated deaths
            cumDeaths = dfSingleCountry['CumDeaths'].to_numpy(copy=True) 
            # the cumulated cases to be shifted by one day
            cumDeathsShifted = dfSingleCountry['CumDeaths'].to_numpy(copy=True) 
            # delete the last element
            cumDeathsShifted = np.delete(cumDeathsShifted, [len(cumDeathsShifted) - 1])
            # insert a 0 as the first element
            cumDeathsShifted = np.insert(cumDeathsShifted, 0, 0)
            # subtract the two to get the new cases
            deaths = cumDeaths - cumDeathsShifted
            # add them to the dataframe
            dfSingleCountry['DailyDeaths'] = deaths
    ...
    def download_CSV_file():
        ...
        preFix = today.strftime('%Y-%m-%d') + "-WHOv1"
        try:
        ...
        if os.path.exists(targetFilename):
            ...
            url = 'https://github.com/WorldHealthOrganization/xform-covid-casecount-who/raw/master/output/admin0/all.csv'
    ...
    def get_data_source_info(self):
        ...
        """
        info = ["World Health Organization v1", 
                "WHOv1",
                "https://github.com/WorldHealthOrganization/xform-covid-casecount-who/tree/master/output/admin0"]
        return info
