...
def test_false_attribute():
    ...
    assert response.status_code == 422
    assert search("value is not a valid enumeration member",json.dumps(response.json()))
...
def test_log_not_boolean():
    ...
    assert response.status_code == 422
    assert search("value could not be parsed to a boolean",json.dumps(response.json()))
